...
Linked is the [original paper](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-014-0550-8) from Anders & Huber introducing the concepts implemented in DESeq2. We will be using the [pasilla package](https://www.bioconductor.org/packages/release/data/experiment/html/pasilla.html) for our example data.
...
```{r check sample names, echo = FALSE, message=FALSE}
...
```{r exploratory pca, fig.height=5, fig.width=5}
...
In this example we see that while controlling the adjustedp/FDR threshold at <0.05, we have 838 DE genes betwen these groups, 406 are more expressed in the treated samples or up-regulated and 432 are more expressed in the untreated samples or down-regulated. Up and down regulation refers to the group you have set as the control or reference group. In this case, we are comparing the treated samples to the untreated samples so the untreated samples are our reference group and we say genes are up or down regulated in comparison to this group.

In our walkthorugh of edgeR we use the same example dataset and found that while controlling the adjustedp/FDR threshold at <0.05, we found no differentailly expressed genes between these groups. 
...
We check that the data frame was created successfully by using the informal unit test of dimension with the expected number of rows and columns and telling the program to stop if the file does not have these dimensions. After this runs successful we would typically export them as a .csv file for you.
