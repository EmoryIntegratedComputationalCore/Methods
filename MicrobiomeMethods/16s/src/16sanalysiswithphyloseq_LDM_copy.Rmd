...
pacman::p_load("devtools","phyloseq","tidyverse","qiime2R", "ggplot2", 
              "reshape2", "GUniFrac", "vegan", "LDM", "janitor", "ggfortify")
...
We also need to recover the data from the phyloseq object and transform the taxa table, asvs, and the metadata objects from phyloseq data objects to data frames for later statistical analyses.
...
metadata <- as.data.frame(sample_data(GP)) %>%
  clean_names() %>%
  mutate(type = as.character(sample_type))
...
To examine this relationship we first specify the data object for our model, called form by stating we would like to assess whether or not a sample being of a different type significantly impacts its microbiome composition while controlling for whether or not that sample comes from a human source. Next, we fit this linear discriminant model using the Bray-Curtis distance. Recall that this distance is one of many metrics available to quantify the between-sample variability or how different each sample group is from one another.

### Fit the LDM {-}

```{r fit LDM, message=FALSE}

form <- asvt | (human) ~type
...
Similarly as above, we will use a scree plot here to quantify how much variability in our model is explained by sample type while controlling for a sample being human or not.
...
color <- c(rep("orange", 1), rep("darkblue", length(scree.VE.freq)-1))
...
Each dot on this plot represents a variable in the model either one we have recorded in our dataset or so-called surrogate variables which we may not have measured. In this scree plot we see results which support our Correspondance Analysis above, that sample type  (also represented by CA1) is contributing ~25% of the observed variability. 

### Global Hypothesis Testing {-}

In order to determine whether sample type (while controlling for a human or non-human source) is statistically significantly contributing to observed differences in microbial composition we first perform a test of the global hypothesis. We want to know, overall, are there differences sample types with regard to the composition of the ASVs expressed in each group?
...
To prepare a PCoA plot from data generated in LDM we adjust the data by a covariate of interest using the Bray Curtis ordination method, calculate the principal components, extract the eigenvectors, and plot the first two PCos uisng the ggfortify library. This plot suggests that half of the variability 

### Visualization {-}
```{r viz, message = FALSE, echo = FALSE}

adj <-adjust.data.by.covariates(formula = ~human , 
                               data = asvt,
                               otu.table= asvt, 
                               dist.method="bray")

PCos <- eigen(adj$adj.dist, symmetric=TRUE)

pco_vectors <- as.data.frame(PCos$vectors) %>%
        mutate(sample_group = as.factor(metadata$human), 
               sample_id = as.factor(metadata$x_sample_id), 
               sample_type = as.factor(metadata$sample_type))
row.names(pco_vectors) <- pco_vectors$sample_id

df <- pco_vectors[c(1, 2)]

(PCoA_LDM <- autoplot(prcomp(df), 
         data = pco_vectors, 
         colour = "sample_group",
         label = FALSE,
         title = "Variability between Human and Non-human Samples"))
