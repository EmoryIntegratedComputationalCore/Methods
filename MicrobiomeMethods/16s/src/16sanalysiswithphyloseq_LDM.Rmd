...
pacman::p_load("devtools","phyloseq","tidyverse","qiime2R", "ggplot2", 
              "reshape2", "GUniFrac", "vegan", "LDM", "ggfortify", "assertr", "janitor")
...
```{r alphadiv, message = FALSE, echo = FALSE}
...
```{r betadiv, message = FALSE, echo = FALSE}
...
```{r taxa bar, message = FALSE, echo = FALSE}
p1  <- 
...
asvs_t <- asvs_t %>%
  mutate(sample_id = row.names(asvs_t)) %>%
  arrange(sample_id)
...
```{r filttaxa, message = FALSE, echo = FALSE}

otu_pres <- which(colSums(asvs_t > 0) >= 5)
...
Above, we colored samples by their type and used different shapes to indicate whether they were from humans or not. Since we are only interested in assessing the impact of whether or not a sample comes from a human, let's look at a Correspondance Analysis plot with samples colored by this variable.

```{r ordplot2, message=FALSE}
...
#FIXME:Error in colSums(otu.table) : 'x' must be numeric
#3.colSums(otu.table)
#2.which(colSums(otu.table) > 0)
#1.ldm(formula = form, data = meta, dist.method = "bray", n.perm.max = 0)
...
resqOrdered <- res[order(res$qvals),]
