...
alpha and beta diversity metrices) we believe that phyloseq in combination with 
...
experimentors. LDM can accomodate both continuous and categorical data. 

There are many ways to import data into phyloseq. We typically import the .qza 
files produced by QIIME2 but in this walkthrough we will be using a built-in 
dataset that you can use anytime so you can follow along with this walkthrough 
...
significance threshold adjusted for the fact that we have assessed muliple 
...
human <- get_variable(GP, "SampleType") %in% c("Feces", "Mock", "Skin", 
"Tongue")
...
sequencing along with your experimenatl samples.

Here we have also specified that we would like the Classic ggplot2 theme,which 
gives a white background with a rainbow of colors for each box plot. This color 
scheme is customizable and we would be happy to use one to fit your project. 
Please see our Data Visualization Menu linked 
[here](https://github.com/EmoryIntegratedComputationalCore/Methods/blob/master/D
ataVisualizationMenu.pdf)
...
(alphadivplot <- plot_richness(GP, "SampleType", measures = c("Shannon")) + 
  geom_boxplot(aes(SampleType, value, fill = SampleType), size = 0.5, alpha = 
0.6) + 
...
Similarly to a Principal Components Analysis a Correspondance Analysis tells us 
how much variability is being contributed to the data set by each axis (like a 
principal component). In the case of this data, we only want to lok at the top 
...
options for Unifrac distance, Jaccard, Manhattan, Euclidean, or Caho metrics 
...
(betadivplot <-plot_ordination(GP, GPca, "samples", color = "SampleType", shape 
= "human") + 
  geom_point(size=3) + 
  theme_minimal())
...
out some additional exmaples from previous projects 
[here](https://github.com/EmoryIntegratedComputationalCore/Methods/blob/master/D
ataVisualizationMenu.pdf)
...
have converted those in the metadata to be all lowercase as well. 
...
the t function to prepare the data in the way we need it and transpose the 
columns into rows and rows into columns. Note that at this point we have `r 
nrow(asvs_t)` samples and `r ncol(asvs_t)` taxa.
...
do appear to have some substantial seperation based on this variable. We should 
...
Since we are performing permuations, we set a seed to be able to reproduce our 
...
like as long as you use the same one everytime you need a seed in this analysis 
...
taxa are significiantly differently abundant between human and non-human 
samples. Here we create a dataframe of results ordered by smallest local FDR 
and examine the top ten.
